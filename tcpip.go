...
// this whole file re-implements the TCPIP functionality of the
// crypto/ssh client to make some patches for ngrok compatibility.
// luckily we can just wrap ssh.Client with these patched methods.
// we don't use them all, but they're here if we need them.
...
func (c *client) ListenTCP(laddr *net.TCPAddr) (net.Listener, error) {
	...
	m := channelForwardMsg{
		"localhost", // NGROK FIX: only works with "localhost"
		uint32(laddr.Port),
	}
	// NGROK FIX: do not wait for reply
	_, resp, err := c.SendRequest("tcpip-forward", false, ssh.Marshal(&m))
	if err != nil {
		return nil, err
	}
...
func (l *forwardList) handleChannels(in <-chan ssh.NewChannel) {
	for ch := range in {
		...
		case "forwarded-tcpip":
			...
			// NGROK FIX
			payload.Addr = "127.0.0.1"
			payload.OriginAddr = strings.Split(payload.OriginAddr, ":")[0]
...
func (c *client) dial(laddr string, lport int, raddr string, rport int) (ssh.Channel, error) {
...
type tcpChan struct {
	ssh.Channel // the backing channel
}
